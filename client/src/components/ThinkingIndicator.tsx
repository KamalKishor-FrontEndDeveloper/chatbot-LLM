...
export default function ThinkingIndicator() {
  ...
  return (
    <div className="flex items-start space-x-3" data-testid="thinking-indicator">
      <div className="w-8 h-8 bg-primary rounded-lg flex items-center justify-center flex-shrink-0 mt-1 animate-pulse ring-1 ring-primary/20">
      ...
      <div className="bg-card border border-border rounded-2xl rounded-tl-md px-4 py-4 shadow-sm min-w-0 flex-1">
        <div className="space-y-3">
          ...
          <div className="flex items-center space-x-3">
            ...
            <span className="text-sm font-medium text-foreground">{thinkingStages[currentStage]?.text}</span>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between text-xs text-muted-foreground">
              <span>
                Step {currentStage + 1} of {thinkingStages.length}
              </span>
            ...
            {thinkingStages.map((stage, index) => (
              <div
                ...
                className={`w-2 h-2 rounded-full transition-colors duration-300 ${
                  index < currentStage ? "bg-primary" : index === currentStage ? "bg-primary animate-pulse" : "bg-muted"
                }`}
              />
            ))}
          </div>
